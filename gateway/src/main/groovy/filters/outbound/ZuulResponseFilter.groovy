    ...
    HttpResponseMessage apply(HttpResponseMessage response) {
        ...
        if (SEND_RESPONSE_HEADERS.get()) {
            ...
            if (statusCategory != null) {
                headers.set(com.netflix.zuul.constants.ZuulHeaders.X_ZUUL_STATUS, statusCategory.name())
            }
            ...
            headers.set(com.netflix.zuul.constants.ZuulHeaders.X_ZUUL_PROXY_ATTEMPTS, headerStr)

            headers.set(com.netflix.zuul.constants.ZuulHeaders.X_ZUUL, "zuul")
            headers.set(com.netflix.zuul.constants.ZuulHeaders.X_ZUUL_INSTANCE, System.getenv("EC2_INSTANCE_ID") ?: "unknown")
            headers.set(com.netflix.zuul.constants.ZuulHeaders.CONNECTION, com.netflix.zuul.constants.ZuulHeaders.KEEP_ALIVE)
            headers.set(com.netflix.zuul.constants.ZuulHeaders.X_ZUUL_FILTER_EXECUTION_STATUS, context.getFilterExecutionSummary().toString())
            headers.set(com.netflix.zuul.constants.ZuulHeaders.X_ORIGINATING_URL, response.getInboundRequest().reconstructURI())

            if (response.getStatus() >= 400 && context.getError() != null) {
                Throwable error = context.getError()
                headers.set(com.netflix.zuul.constants.ZuulHeaders.X_ZUUL_ERROR_CAUSE,
